...
- «лет до порога 30% доли 65+» и «естественная динамика 2019->2039» -
  ...
  рождаемость базового сценария прогноза проекта): показывает, где
...
# фиксированные e0 контрфакта: уровень базовой траектории прогноза (WPP medium)
...
def main() -> None:
    ...
    (OUT / "aging.json").write_text(json.dumps({
        ...
        "threshold": 30,
        "counterfactual": "нулевая миграция; смертность/рождаемость базового сценария прогноза (e0 м 72,0 / ж 81,5; областные ASFR-2018)",
        "territories": out,
    }, ensure_ascii=False))
