...
## Прогноз населения 2026–2075 (v2026.1)

Этап MVP (страна, области, Минск): когортно-компонентный прогноз с тремя
сценариями и 80% интервалом. На карте — слайдер за 2026 год (зона прогноза
выделена штриховкой, переключатель сценариев, тултипы с версией и сценарием).

```bash
make forecast          # прогон + бэктест 2009->2019 + чувствительность
```

Гейты (TASK_SPEC WP-F4/F5): калибровка base-2050 = +1,0% к медиане UN WPP
2024 (порог ±3%); бэктест — национальный итог −0,41% (порог ±2%), MAPE
областей 2,39% против 4,13% у наивной экстраполяции. Отчёт —
[docs/notes/validation.md](docs/notes/validation.md); пакет —
`by-maps-forecast-v1.0.0.zip`.
