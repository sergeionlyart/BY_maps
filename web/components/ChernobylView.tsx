...
export default function ChernobylView() {
  ...
  // Переводчик: именуем tr, т.к. локальная t ниже — это data.territories.
  const tr = useT();

  if (!cher || !data || !pair) return <p className="hint">{tr('Загрузка данных…')}</p>;

  const t = data.territories;
  const chart: ChartSeries[] = [
    { name: `${pair.ru} (${tr('класс')} ${pair.klass})`, color: AFF_COLOR, points: indexSeries(t[pair.id], cher.baseYear) },
    { name: `${pair.controlRu} (${tr('контроль')})`, color: CAT[0], points: indexSeries(t[pair.control], cher.baseYear) },
  ];
    ...
    <div>
      <div className="controls" style={{ marginBottom: 6 }}>
        ...
        <a className="btn" href="/artifacts/by-maps-chernobyl-v1.0.0.zip" download>
          ⬇ {tr('Проверяемый пакет (ZIP)')}
        </a>
      </div>

      <div className="stat-row">
        <div className="stat-tile">
          <div className="st-label">{tr('Зона эвакуации/отселения (класс 1)')}</div>
          <div className="st-value">{k1.map((p) => p.ru.replace(' район', '')).join(', ')}</div>
          <div className="st-delta">{tr('эвакуация 1986 г.; ныне — Полесский заповедник')}</div>
        </div>
        <div className="stat-tile">
          <div className="st-label">{tr('Закрытые отселённые территории')}</div>
          <div className="st-value">{totalClosedKm2.toLocaleString('ru-RU')} км²</div>
          <div className="st-delta">{tr('в 12 районах классов 1–2 (КПП-режим, 2022)')}</div>
        </div>
        <div className="stat-tile">
          <div className="st-label">{tr('НП в зонах загрязнения')}</div>
          <div className="st-value">2193 → 2022</div>
          <div className="st-delta">{tr('перечни 2016 и 2021 гг. (зоны сужаются)')}</div>
        </div>
      </div>

      <div className="chart-block">
        <div className="chart-title">
          {tr('Население, индекс: перепись')} {cher.baseYear} = 100 · {pair.ru} {tr('против контроля')}
          {' · '}
          <a href={`/map?sel=${pair.id}`}>{tr('показать на карте')}</a>
        </div>
        <div className="controls" style={{ margin: '2px 0 6px' }}>
          <select value={pair.id} onChange={(e) => select(e.target.value)} aria-label={tr('пара район-контроль')}>
            <optgroup label={`${tr('Класс 1 —')} ${cher.classLabels['1']}`}>
              {cher.pairs.filter((p) => p.klass === 1).map((p) => (
                <option key={p.id} value={p.id}>{p.ru} ↔ {p.controlRu}</option>
              ))}
            </optgroup>
            <optgroup label={`${tr('Класс 2 —')} ${cher.classLabels['2']}`}>
      ...
      <div className="stat-row">
        <div className="stat-tile">
          <div className="st-label">{pair.ru}: {tr('население 2019 к 1979')}</div>
          <div className="st-value">{idx2019(t[pair.id], pair.pop1979)?.toFixed(0)}%</div>
          <div className="st-delta down">{tr('контроль')} ({pair.controlRu}): {idx2019(t[pair.control], pair.controlPop1979)?.toFixed(0)}%</div>
        </div>
        {pair.closedHa != null && (
          <div className="stat-tile">
            <div className="st-label">{tr('Закрытые отселённые территории')}</div>
            <div className="st-value">{Math.round(pair.closedHa / 100).toLocaleString('ru-RU')} км²</div>
            <div className="st-delta">{tr('КПП-режим, 2022 (табл. 12 МЧС)')}</div>
          </div>
        )}
        <div className="stat-tile">
          <div className="st-label">{tr('НП в зонах загрязнения')}</div>
          ...
          <div className="st-delta">
            {tr('перечень-2016 → перечень-2021')}
            {pair.np2021.POSL ? ` · ${pair.np2021.POSL} ${tr('НП — зона последующего отселения')}` : ''}
          </div>
        </div>
      </div>

      <div className="chart-block">
        <div className="chart-title">{tr('Все пары: отставание от контроля к 2019 г. (проценты индекса 1979 = 100)')}</div>
        <div className="zone-table-wrap">
        <table className="zone-table">
          <thead>
            <tr>
              <th>{tr('Район')}</th><th>{tr('Класс')}</th><th>{tr('Закрыто, км²')}</th>
              <th>{tr('НП в зонах 2016→2021')}</th><th>{tr('Контроль')}</th><th>{tr('Отставание')}</th>
            </tr>
          </thead>
          <tbody>
            {cher.pairs.map((p) => {
        ...
        <p className="hint" style={{ marginTop: 4 }}>
          {tr('Отрицательное отставание — район сократился сильнее контроля. Исключения информативны: Добрушский и Костюковичский держались лучше контролей за счёт райцентров-городов.')}
        </p>
      </div>

      <p className="src-note">
        {tr('Классификация — по официальным перечням НП в зонах радиоактивного загрязнения (')}{cher.npa.current}{tr('; ранее ')}{cher.npa.y2016}{tr(') и данным о закрытых территориях с контрольно-пропускным режимом (МЧС, «Беларусь и Чернобыль: 36 лет», табл. 12). Пограничные случаи вне классов:')}{' '}
        {Object.entries(cher.borderline).map(([k, v]) => `${k} — ${v}`).join('; ')}{tr('. Контроль подобран по населению переписи-1979 среди районов вне перечней. Полные ограничения — в методблоке и LIMITATIONS.md пакета.')}
      </p>
    </div>
  );
}
