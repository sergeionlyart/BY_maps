...
export default function ForecastArtifactBody() {
  const t = useT();
  const be = useLang() === 'be';
  ...
  return (
    <div className="page">
      <div className="page-breadcrumb">
        <Link href={p('/artifacts')}>{t('Артефакты')}</Link> · {t('Прогноз')}
      </div>
      <h1>{t('Пакет: Прогноз населения 2026–2075 (v2026.4)')}</h1>
      <p className="page-lead">
        {t('Прогноз всех уровней: страна, 6 областей, Минск (CCMPP), 118 районов (Гамильтон–Перри с IPF-согласованием), ~200 городов (доли в районах, облцентры — собственные когортные модели); два стартовых ряда — official и adjusted (поправка на незарегистрированную эмиграцию 2020–2026 из зеркальной статистики, WP-F3). Три сценария с обоснованием каждого параметра, вероятностный веер (сеяный Монте-Карло траекторий СКР/ОПЖ, калиброванный по 80% PI WPP; уровни 0–1), три бэктеста с гейтами. Прогон детерминирован (сид фиксирован) — воспроизведение даёт байт-в-байт те же числа.')}
      </p>
      ...
      <div className="card">
        <div className="card-code">v1.2.0 · 2026-07-11 · {t('git-тег')} artifact-forecast-v1.2.0 · {t('прогноз')} v2026.3</div>
        <p>
          {t('WP-F3: второй стартовый ряд adjusted — официальный старт минус интервальная оценка незарегистрированного оттока 2020–2026 (178–416 тыс., центральная 270 тыс.; зеркальная статистика Eurostat/UDSC/Литвы/Грузии + опросные доли диаспоры). Гейт: интервал накрывает сток ВНЖ ЕС (253 тыс.) и ориентир МВД (350 тыс.). Official-ряды идентичны v1.1.0. 26 метрик.')}
        </p>
        <div className="card-foot">
      ...
      <div className="card">
        <div className="card-code">v1.1.0 · 2026-07-11 · {t('git-тег')} artifact-forecast-v1.1.0 · {t('прогноз')} v2026.2</div>
        <p>
          {t('Этап 5: добавлены уровни 2–3. Районы — CCR 2009→2019 со shrinkage и чернобыльским floor; города — логистические доли; 5 облцентров — CCMPP. Гейты: бэктест районов 2019→2026 MAPE 2,12% против 2,76% наивной; бэктест городов 6,7% против 10,9%. Уровни 0–1 идентичны v2026.1. 19 контрольных метрик.')}
        </p>
        <div className="card-foot">
      ...
      <div className="card">
        <div className="card-code">v1.0.1 · 2026-07-11 · {t('git-тег')} artifact-forecast-v1.0.1 · {t('прогноз')} v2026.1</div>
        <p>
          {t('Исправление входных данных по итогам агентного аудита этапа 4: устранена коллизия territory_id в age2009/age2019.csv (Октябрьский район Минска против Октябрьского района Гомельской области). Все результаты прогноза идентичны v1.0.0 — коллизия на модель не влияла.')}
        </p>
        <div className="card-foot">
      ...
      <div className="card">
        <div className="card-code">v1.0.0 · 2026-07-11 · {t('git-тег')} artifact-forecast-v1.0.0 · {t('прогноз')} v2026.1</div>
        <p>
          {t('Первый релиз (этап MVP). Гейты: калибровка base-2050 = +1,0% к медиане UN WPP 2024 (порог ±3%); бэктест — национальный итог −0,41% (порог ±2%), MAPE 2,39% против 4,13% у наивной экстраполяции. 11 контрольных метрик.')}
        </p>
        <div className="card-foot">
...
# Все 30 контрольных метрик воспроизведены в допусках.`}</code></pre>
      ...
      <p className="hint">
        {t('Методика — кнопка «Методика прогноза» на ')}
        <Link href={p('/map')}>{t('карте')}</Link>
        {t(' (в прогнозной зоне слайдера) или файл methods/forecast.md; валидация — docs/notes/validation.md внутри пакета.')}
      </p>
    </div>
  );
}
