...
export default function ContentDoc({ body, toc = false, lang = 'ru', footer, articlesNav }: {
  body: string;
  toc?: boolean;
  lang?: 'ru' | 'be';
  /** Доп. блок после статьи в той же читаемой колонке (напр. AuthorCard). */
  footer?: React.ReactNode;
  /** Список статей (ArticlesMenu) - встаёт в ту же боковую колонку, над
   *  оглавлением текущей страницы. Используется только страницами /article/*. */
  articlesNav?: React.ReactNode;
}) {
  const h2 = toc ? headings(body) : [];
  return (
    <div className="page content-page" lang={lang}>
      {(articlesNav || h2.length > 1) && (
        <nav className="content-toc" aria-label="Оглавление">
          {articlesNav}
          {h2.length > 1 && (
            <>
              {articlesNav && <hr className="content-toc-sep" />}
              <div className="content-toc-title">{lang === 'be' ? 'Змест' : 'Содержание'}</div>
              <ul className="content-toc-headings">
                {h2.map((h) => (
