  ...
  return (
    <div className="page">
      <div className="page-breadcrumb">
        <Link href="/artifacts">Артефакты</Link> · INF-01
      </div>
      <h1>Пакет: Иерархия городов и закон Ципфа</h1>
      <p className="page-lead">
        Проверяемый пакет исследования{' '}
        <Link href="/research/zipf">«Иерархия городов и закон Ципфа, 1897–2026»</Link>.
        Один источник (222 города, переписи 1897–2019 + оценки), весь расчёт —
        стандартная библиотека Python, воспроизведение ~10 секунд.
      </p>

      <h2>Версии</h2>
      {VERSIONS.map((v) => (
        <div className="card" key={v.version}>
          <div className="card-code">v{v.version} · {v.date} · git-тег {v.tag}</div>
          <p>{v.changes}</p>
...
# Все 8 контрольных метрик воспроизведены в допусках.`}</code></pre>
      ...
      <p className="hint">
        Валидация в CI: каждый пуш пересобирает пакет и сверяет с опубликованным
        байт-в-байт; полный прогон run.sh — часть пайплайна.
      </p>
    </div>
  );
}
