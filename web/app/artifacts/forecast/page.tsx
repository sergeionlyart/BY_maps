...
export const metadata: Metadata = {
  ...
  description: 'Проверяемый пакет прогноза населения Беларуси 2026–2075 (v2026.3, уровни 0–3, ряды official/adjusted).',
};
...
export default function ForecastArtifactPage() {
  return (
    <div className="page">
      ...
      <h1>Пакет: Прогноз населения 2026–2075 (v2026.3)</h1>
      <p className="page-lead">
        ...
        обоснованием каждого параметра, 80% интервал (уровни 0–1), три
        бэктеста с гейтами. Прогон детерминирован — воспроизведение даёт
        байт-в-байт те же числа.
      </p>
      ...
      <h2>Состав (v1.2.0)</h2>
      <pre><code>{`by-maps-forecast-v1.2.0/
...
│   │                                города (sub.py) + бэктесты + реконструкция
│   │                                оттока (mirror.py)
...
├── data/curated/forecast_v2026_3.csv · web/public/data/forecast.json   итоги
...
└── checks/                          инварианты, ожидаемые результаты, chksums`}</code></pre>
      ...
      <pre><code>{`unzip by-maps-forecast-v1.2.0.zip && cd by-maps-forecast-v1.2.0
pip install -r code/requirements.lock   # PyYAML
bash code/run.sh
# == 1/7 Реконструкция оттока (WP-F3): интервал и adjustment.csv ==
# == 2/7 Прогон прогноза (3 сценария, уровни 0-3, ряды official/adjusted) ==
